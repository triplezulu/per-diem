...
export default function App(){
  ...
  async function syncFl3xxCalendar(){
    ...
    try {
      ...
      const reportLegs=result.legs.filter(l=>legOverlapsDateRange(l,fl3xxFromDate,fl3xxToDate));

      if(!reportLegs.length){
        const msg=result.stats.on>0 && result.stats.flights===0 && result.stats.trv===0
          ...
          : "No importable FL3XX Flight/TRV movements found in the selected range.";
        setIcsImportStatus(msg);
        return;
      }

      // Replace FL3XX data across the context window, not just inside the month.
      // The context movements stay in memory so a trip beginning on 31 Jul can
      // correctly continue into the August report.
      setLegs(prev=>replaceImportedLegsForRange(prev,result.legs,context.from,context.to));

      const reportFlights=reportLegs.filter(l=>l.movementType==="FLIGHT").length;
      const reportTrv=reportLegs.filter(l=>l.movementType==="TRV").length;
      const details=`Synced ${reportFlights} flight(s), ${reportTrv} TRV proceeding(s) for ${fl3xxFromDate} → ${fl3xxToDate}. Boundary context: ${context.from} → ${context.to}.`;
  ...
  const calcAll=useMemo(()=>buildDailyPerDiems(legs,rates,homes,breakfastByDate),[legs,rates,selectedEmp,breakfastByDate]);
  const calc=useMemo(()=>({
  ...
  const monthLegs=legs.filter(l=>legOverlapsDateRange(l,fl3xxFromDate,fl3xxToDate));
