...
export default function App(){
  ...
  async function syncFl3xxCalendar(){
    ...
    try {
      ...
      if(archiveMode){
        details+=` Server archive active (${Number(res.headers.get("X-FL3XX-Archive-Count")||0)} event(s)).`;
        ...
        details+=` Server archive is not active; using live FL3XX/local cache only.`;
      }
  ...
  <div className="mx-auto max-w-6xl p-4 md:p-6 space-y-4">
    ...
    {(feedCoverage||fl3xxArchive.length>0||serverArchiveMode) && <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500 px-1">
      {serverArchiveMode&&<span className="font-medium text-emerald-700">D1 server archive: active</span>}
      {serverArchiveCoverage&&<span>Archived timed coverage: <strong>{serverArchiveCoverage.from} → {serverArchiveCoverage.to}</strong></span>}
      {serverArchiveMode&&<span>Server archive: <strong>{serverArchiveCount}</strong> event(s)</span>}
      {liveTimedCoverage&&<span>Live FL3XX timed feed: <strong>{liveTimedCoverage.from} → {liveTimedCoverage.to}</strong></span>}
      <span>Browser cache: <strong>{fl3xxArchive.length}</strong> movement(s)</span>
      ...
      {serverArchiveCoverage&&fl3xxFromDate<serverArchiveCoverage.from&&<span className="text-amber-700">Selected range starts before the D1 archive history.</span>}
    </div>}
    {rawDiag&&<div className={`rounded-xl border px-3 py-2 text-xs ${rawDiag.unparsedTimedCount>0 || (rawDiag.rawTimedFrom && feedCoverage?.from && rawDiag.rawTimedFrom<feedCoverage.from) ? "border-amber-200 bg-amber-50 text-amber-900" : "border-slate-200 bg-white text-slate-600"}`}>
      <strong>Returned archive/feed diagnostics:</strong> {rawDiag.eventCount} VEVENT(s)
          ...
          <details className="rounded-xl border border-slate-200 bg-slate-50/70 px-3 py-2">
            ...
            <div className="mt-3">
              ...
              <div className="mt-4 border-t border-slate-200 pt-3">
                <div className="text-sm font-medium">FL3XX archive tools</div>
                <div className="mt-1 text-xs text-slate-500">The D1 server archive is the permanent history. Browser archive tools remain available as a fallback. Historical FL3XX .ics snapshots can be imported into D1 and are never removed just because they disappear from the current live feed.</div>
                <div className="mt-2 flex flex-wrap gap-2">
                  ...
                  <Button type="button" variant="secondary" onClick={exportFl3xxArchive} disabled={!fl3xxArchive.length}>Backup browser cache</Button>
                  <Button type="button" variant="secondary" onClick={downloadRawFl3xxFeed} disabled={!rawFeedText}>Download returned ICS</Button>
