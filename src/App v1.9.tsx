...
export default function App(){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        ...
        if(raw){
          ...
          if(Array.isArray(arr)&&arr.length){
            setRates(arr);
            const yr=Number(localStorage.getItem(LS_RATES_YEAR_KEY)||"2025");
            setRatesYear(Number.isFinite(yr)?yr:2025);
      ...
      for(const url of DEFAULT_RATE_CANDIDATES){try{const res=await fetch(url,{cache:"no-store"});if(!res.ok)continue;const cleaned=normalizeRates(await res.json());if(cleaned.length&&!cancelled){setRates(cleaned);setRatesYear(2025);setRatesStatus("loaded");try{localStorage.setItem(LS_RATES_KEY,JSON.stringify(cleaned));localStorage.setItem(LS_RATES_YEAR_KEY,"2025");}catch{}return;}}catch{}}
  ...
  <div className="mx-auto max-w-6xl p-4 md:p-6 space-y-4">
    ...
    <div className="flex items-center gap-3 flex-wrap">
      <Card className="p-3 flex items-center gap-3">
        <Badge className={ratesYearMismatch?"border-amber-300 bg-amber-50 text-amber-800":""}>rates: {ratesStatus}{ratesYear?` · ${ratesYear}`:" · year unknown"}</Badge>
        <label className="text-sm cursor-pointer"><input type="file" accept=".json,.csv" className="hidden" onChange={e=>{const f=e.target.files?.[0];if(f)onUploadRatesFile(f);}}/><span className="inline-flex items-center gap-2"><IconUpload className="h-4 w-4"/> Load rates</span></label>
      </Card>
    ...
    {ratesYearMismatch && <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
      <strong>Rates year mismatch:</strong> selected month is {selectedYear}, but loaded per-diem rates are marked as {ratesYear}. Load the correct rates before finalizing the report.
    </div>}
