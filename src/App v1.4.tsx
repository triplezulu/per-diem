...
export default function App(){
  ...
  const [fl3xxProxyUrl,setFl3xxProxyUrl]=useState("");
  ...
  useEffect(()=>{
    try {
      ...
      if(raw){
        ...
        if(typeof v?.proxyUrl==="string") setFl3xxProxyUrl(v.proxyUrl);
  ...
  async function syncFl3xxCalendar(){
    ...
    try {
      ...
      const events=parseIcsCalendar(body,rates);
      ...
      const details=`Synced ${result.stats.flights} flight(s), ${result.stats.trv} TRV proceeding(s). Ignored ${result.stats.on} ON event(s).`;
  ...
  return <div className="mx-auto max-w-6xl p-4 space-y-4">
    ...
    <div className="flex items-center gap-3 flex-wrap">
      ...
      <Card className="p-3 flex items-center gap-3"><label className="text-sm cursor-pointer"><input type="file" accept=".ics,text/calendar" multiple className="hidden" onChange={e=>{void onImportIcsFiles(e.target.files);e.currentTarget.value="";}}/><span className="inline-flex items-center gap-2"><IconUpload className="h-4 w-4"/> Import FL3XX ICS</span></label></Card>
      <Card className="p-3 flex items-center gap-2">
        <Button variant="outline" onClick={()=>setFl3xxPanelOpen(v=>!v)}>FL3XX connection</Button>
    ...
    {icsImportStatus && <div className="rounded-xl border bg-neutral-50 px-3 py-2 text-xs text-neutral-700">{icsImportStatus}</div>}

    {fl3xxPanelOpen && <Card><CardHeader><CardTitle>FL3XX Calendar Connection</CardTitle></CardHeader><CardContent>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div className="md:col-span-2"><label className="mb-1 block text-sm font-medium">Proxy URL</label><Input placeholder="https://your-worker.workers.dev/sync" value={fl3xxProxyUrl} onChange={e=>setFl3xxProxyUrl(e.target.value)}/><div className="mt-1 text-xs text-neutral-500">One-time setup. This is the small secure proxy that talks to FL3XX.</div></div>
    ...
    <Card><CardHeader className="flex items-center justify-between"><CardTitle>Trip Legs (UTC)</CardTitle><div className="flex gap-2"><Button variant="secondary" className="gap-2" onClick={addLeg}><IconPlus className="h-4 w-4"/> Add new leg</Button><Button variant="secondary" className="gap-2" onClick={addNextLeg}><IconPlus className="h-4 w-4"/> Next leg</Button></div></CardHeader>
      <CardContent className="space-y-3">{legs.map(leg=>{const invalid=new Date(leg.startUtc+":00Z")>=new Date(leg.endUtc+":00Z");return <div key={leg.id} className="grid grid-cols-1 lg:grid-cols-12 gap-2 items-end">
        ...
        <div className="lg:col-span-2 flex items-center justify-end gap-2">{leg.movementType&&<Badge>{leg.movementType}</Badge>}{invalid&&<span className="text-xs text-red-600">Start must be earlier than End</span>}<Button variant="outline" className="gap-2" onClick={()=>removeLeg(leg.id)}><IconTrash className="h-4 w-4"/> Remove</Button></div>
      </div>})}</CardContent>
    </Card>
